...
const galaxyTexture = textureLoader.load("https://threejs.org/examples/textures/galaxy_starfield.png");
const floorTexture = textureLoader.load("https://threejs.org/examples/textures/brick_diffuse.jpg");

// Repeat the textures for better tiling
...
pointLight.position.set(0, -5, 0); // Below the platform
...
floor.position.y = 0; // Floating in space
scene.add(floor);
...
function floatPlatform() {
    floor.position.y += floatSpeed * floatDirection;
    if (floor.position.y > 0.2 || floor.position.y < -0.2) {
        floatDirection *= -1;
    }
}
